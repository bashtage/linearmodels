...
from linearmodels.panel.covariance import ClusteredCovariance, HeteroskedasticCovariance, \
    HomoskedasticCovariance, CovarianceManager, DriscollKraay
...
class TestCovariance(object):
    ...
    def test_heteroskedastic_smoke(self):
        cov = HeteroskedasticCovariance(self.y, self.x, self.params, self.time_ids, extra_df=0).cov
        assert cov.shape == (self.k, self.k)
        cov = HeteroskedasticCovariance(self.y, self.x, self.params, self.time_ids, extra_df=0).cov
        assert cov.shape == (self.k, self.k)

    def test_homoskedastic_smoke(self):
        cov = HomoskedasticCovariance(self.y, self.x, self.params, self.time_ids, extra_df=0).cov
        assert cov.shape == (self.k, self.k)
        cov = HomoskedasticCovariance(self.y, self.x, self.params, self.time_ids, extra_df=0).cov
        assert cov.shape == (self.k, self.k)

    def test_clustered_covariance_smoke(self):
        cov = ClusteredCovariance(self.y, self.x, self.params, self.time_ids, extra_df=0).cov
        assert cov.shape == (self.k, self.k)

        cov = ClusteredCovariance(self.y, self.x, self.params, self.time_ids, extra_df=0,
    ...
    def test_clustered_covariance_error(self):
        with pytest.raises(ValueError):
            ClusteredCovariance(self.y, self.x, self.params, self.time_ids, extra_df=0,
    ...
    def test_driscoll_kraay(self):
        cov = DriscollKraay(self.y, self.x, self.params, self.time_ids).cov
        assert cov.shape == (self.k, self.k)
        cov = DriscollKraay(self.y, self.x, self.params, self.time_ids, kernel='parzen').cov
        assert cov.shape == (self.k, self.k)
