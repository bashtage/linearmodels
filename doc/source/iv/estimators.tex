...
and so it is not possibly to use a closed form estimator. 
...
\[
Pr\left(z>Z\right)=2-2\Phi\left(\left|z\right|\right)
\]
If the covariance estimator was debiased, a Student's t distribution
with $n-k$ degrees of freedom is used, 

\begin{align*}
Pr\left(z>Z\right) & =2-2t_{n-k}\left(\left|z\right|\right)
\end{align*}
...
\]
where $q_{\alpha/2}$ is the $\alpha/2$ quantile of a standard Normal
distribution or a Student's t. The Student's t is uses when a debiased
